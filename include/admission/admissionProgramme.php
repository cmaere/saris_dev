<?php
	...
	if (array_key_exists('arr', $_GET)) 
	{
		 ...
		 foreach( $arr as $i ) 
		 {
			 if ($i !="")
			 {
				 $sqldelete="DELETE FROM  programme WHERE ProgrammeID=$i";
	...
	if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "frmInstEdit")) {
	  $updateSQL = sprintf("UPDATE programme SET ProgrammeCode=%s, ProgrammeName=%s, Title=%s, Faculty=%s WHERE ProgrammeID=%s",
	                       GetSQLValueString($_POST['txtCode'], "text"),
			       GetSQLValueString($_POST['txtName'], "text"),
	                       GetSQLValueString($_POST['txtTitle'], "text"),
	                       GetSQLValueString($_POST['txtFac'], "text"),
...
exit;
	...
	$column_titles = array('Programme Code','Degree','Faculty');
	...
	$template->setVariable("PAGENAME", "Programme Information");	
	$template->setVariable("LINK", "./?page=$page&section=$section&new=1");
	$template->setVariable("BUTTONNAME", "Add New Programme");		
	...
	if (array_key_exists('new', $_GET) && @$new==1)
	{
		...
		require_once("include/admission/admissionProgramme_new.php");
	}
	else if (isset($_GET['edit']))
	{
		require_once("include/admission/admissionProgramme_edit.php");
	}
	else
	{
		...
		do 
		{ 
		 	$id = $row_inst['ProgrammeID'];
			$programmename = $row_inst['Title'];
			$programmecode = $row_inst['ProgrammeCode'];
			$faculty = $row_inst['Faculty'];
			$template->setCurrentBlock("id");
			$template->setVariable("ID", $id);
			$template->parseCurrentBlock();	
			$template->setCurrentBlock("innercolumns");
			$template->setVariable("ROWDATA", $programmecode);
			$template->parseCurrentBlock();	
			$template->setCurrentBlock("innercolumns");
			$template->setVariable("ROWDATA", $programmename);
			$template->parseCurrentBlock();	
			$template->setVariable("ROWDATA", $faculty);
			$template->parseCurrentBlock();	
