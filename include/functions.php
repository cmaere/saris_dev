...
function spinner()
{
	return("<span class='th-sort'><i class='fa fa-sort-down text'></i> <i class='fa fa-sort-up text-active'></i> <i class='fa fa-sort'></i> </span> ");
}

function pagenumber($template,$pageNum_inst,$page,$section,$totalPages_inst,$queryString_inst)
{
	
	$template->setCurrentBlock("back");
	
	
	if(($pageNum_inst - 1) < 0)
	{
		$template->setVariable("CLASS", "disabled" );
		$template->setVariable("LINK", "#" );
	}
	else
	{
		$maxpage = max(0, $pageNum_inst - 1);
		$template->setVariable("LINK", "./?page=$page&section=$section&pageNum_inst=$maxpage$queryString_inst" );
	...
	$template->parseCurrentBlock();	
        ...
        if($pageNum_inst <> 0)
        {
		$template->setCurrentBlock("pages");
       	 	for($i=1; $i<=$pageNum_inst; $i++)
       	 	{
       	 
       	 	       
		       $template->setVariable("CLASS", "disabled" );
		       $link = "./?page=$page&section=$section&pageNum_inst=".min($totalPages_inst, $i - 1).$queryString_inst;
		       
		       $template->setVariable("LINK", $link );
		       $template->setVariable("PAGES", $i );
		       $template->parseCurrentBlock();	
	...
	$template->setVariable("ACTIVEPAGE", $activepage );
	$template->parseCurrentBlock();	
	
	$lastpage = $pageNum_inst;
	$template->setCurrentBlock("pagesfront");
	for($i=$pageNum_inst+2; $i<=$totalPages_inst; $i++)
	{	 	
		$link = "./?page=$page&section=$section&pageNum_inst=".min($totalPages_inst, $pageNum_inst + 1).$queryString_inst; 
		
		$template->setVariable("LINK", $link );	
		$template->setVariable("FRONTPAGES", $i );
		$template->parseCurrentBlock();	
		$lastpage = $lastpage + 1;
		
	}
	$template->setCurrentBlock("front");
	if($activepage == $totalPages_inst )
	{
 		$template->setVariable("CLASS", "disabled" );
 		$template->setVariable("LINK", "#" );
...
function cha_activesection($regno,$module)
{	
...
$noFound = mysql_num_rows($result);
	while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) 
	{
			...
			if($noFound <> 0)
			  ...
			  echo "<li class='dropdown-submenu'> <a href='#' class='dropdown-toggle' data-toggle='dropdown'> <i class='fa $icon'></i> <span>$sectionname</span> </a>
		   	...
		   	WHERE functions.sectionpage = '$sectionpage'";
	       		...
	       		$result2 = mysql_query($sql2) or die("query error<br>");
...
function Wysiwyg($name,$addr)
{
?>
		
<div class="col-sm-10"> 
	<div data-target="#editor" data-role="editor-toolbar" class="btn-toolbar m-b-sm btn-editor"> 
		<textarea name="<?php echo $name; ?>" style="overflow:scroll;height:150px;max-height:150px" class="form-control"><?php echo $addr; ?> </textarea> </div>
	</div> 
<?php
}
...
function studentindex($username)
{
	...
	$profile = array($username,$position,$registered,$email);
